...
func (h *AskHandler) handleBrowserActionLaunch(msg *types.ClineMessage, dc *DisplayContext) error {
	...
	err := dc.Renderer.RenderMessage("BROWSER", fmt.Sprintf("Sentinel wants to launch browser and navigate to: %s. Approval required.", url), true)
	h.showApprovalHint(dc)
	return err
}
...
func (h *AskHandler) handleUseMcpServer(msg *types.ClineMessage, dc *DisplayContext) error {
	...
	err := dc.Renderer.RenderMessage("MCP",
		fmt.Sprintf("Sentinel wants to %s on the %s MCP server", operation, mcpReq.ServerName), true)
...
func (h *AskHandler) handleNewTask(msg *types.ClineMessage, dc *DisplayContext) error {
	return dc.Renderer.RenderMessage("NEW TASK", fmt.Sprintf("Sentinel wants to start a new task: %s. Approval required.", msg.Text), true)
}
...
func (h *AskHandler) handleCondense(msg *types.ClineMessage, dc *DisplayContext) error {
	return dc.Renderer.RenderMessage("CONDENSE", fmt.Sprintf("Sentinel wants to condense the conversation: %s. Approval required.", msg.Text), true)
}
...
func (h *AskHandler) handleReportBug(msg *types.ClineMessage, dc *DisplayContext) error {
	...
	if err := json.Unmarshal([]byte(msg.Text), &bugData); err != nil {
		return dc.Renderer.RenderMessage("BUG REPORT", fmt.Sprintf("Sentinel wants to create a GitHub issue: %s. Approval required.", msg.Text), true)
	}

	err := dc.Renderer.RenderMessage("BUG REPORT", "Sentinel wants to create a GitHub issue:", true)
	if err != nil {
