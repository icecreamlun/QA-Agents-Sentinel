...
export class AuthService {
	...
	getActiveOrganizationId(): string | null {
		...
		const activeOrg = this._clineAuthInfo.userInfo.organizations.find(
			(org) => org.active,
		);
	...
	private async internalGetAuthToken(
		...
		try {
			...
			if (
				...
				this._clineAuthInfo.provider !== provider.name
			) {
				// Not authenticated
				return null;
			}
			...
			if (
				...
				this._refreshPromise = (async () => {
					...
					try {
						const updatedAuthInfo = await provider.retrieveClineAuthInfo(
							this._controller,
						);
						if (updatedAuthInfo) {
	...
	async createAuthRequest(strict = false): Promise<ProtoString> {
		...
		this._controller.stateManager.setSecret(
	...
	async handleAuthCallback(
		...
		try {
			...
			const expectedState =
				this._controller.stateManager.getSecretKey("axolotlAuthState");
			if (expectedState && state && state !== expectedState) {
				throw new Error("Auth state mismatch");
			}
			const codeVerifier = this._controller.stateManager.getSecretKey(
				"axolotlAuthCodeVerifier",
			);

			this._clineAuthInfo = await this._provider.signIn(
				this._controller,
				authorizationCode,
				codeVerifier,
				refreshToken,
