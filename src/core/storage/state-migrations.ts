...
export async function migrateWorkspaceToGlobalStorage(
	...
	for (const key of keysToMigrate) {
		...
		if (workspaceValue !== undefined && globalValue === undefined) {
			console.log(
				`[Storage Migration] migrating key: ${key} to global storage. Current value: ${workspaceValue}`,
			);
			...
			console.log(
				`[Storage Migration] migrated key: ${key} to global storage. Current value: ${newWorkspaceValue}`,
			);
		}
	}
}

export async function migrateTaskHistoryToFile(
	context: vscode.ExtensionContext,
) {
	try {
		// Get data from old location
		const vscodeGlobalStateTaskHistory = context.globalState.get<
			HistoryItem[] | undefined
		>("taskHistory");

		// Normalize old location data to array (empty array if undefined/null/not-array)
		const oldLocationData = Array.isArray(vscodeGlobalStateTaskHistory)
			? vscodeGlobalStateTaskHistory
			: [];
		...
		if (!Array.isArray(successfullyWrittenData)) {
			console.error(
				"[Storage Migration] Failed to write taskHistory to file: Written data is not an array",
			);
			return;
		}
		...
		console.error(
			"[Storage Migration] Failed to migrate task history to file:",
			error,
		);
	}
}

export async function migrateMcpMarketplaceEnableSetting(
	mcpMarketplaceEnabledRaw: boolean | undefined,
): Promise<boolean> {
...
export async function migrateEnableCheckpointsSetting(
	enableCheckpointsSettingRaw: boolean | undefined,
): Promise<boolean> {
	const config = vscode.workspace.getConfiguration("cline");
...
export async function migrateCustomInstructionsToGlobalRules(
	...
	try {
		const customInstructions = (await context.globalState.get(
			"customInstructions",
		)) as string | undefined;
		...
		if (customInstructions?.trim()) {
			...
			try {
				...
				console.log(
					`Successfully ${existingContent ? "appended to" : "created"} migration file: ${migrationFilePath}`,
				);
			} catch (fileError) {
			...
			console.log(
				"Successfully migrated custom instructions to global Cline rules",
			);
		}
	} catch (error) {
		console.error(
			"Failed to migrate custom instructions to global rules:",
			error,
		);
...
export async function migrateLegacyApiConfigurationToModeSpecific(
	...
	try {
		...
		const planModeApiProvider = await context.globalState.get(
			"planModeApiProvider",
		);
		if (planModeApiProvider !== undefined) {
			console.log(
				"Legacy API configuration migration already completed, skipping...",
			);
			return;
		}

		console.log(
			"Starting legacy API configuration migration to mode-specific keys...",
		);

		// Get the planActSeparateModelsSetting to determine migration strategy
		const planActSeparateModelsSetting = (await context.globalState.get(
		...
		const thinkingBudgetTokens = await context.globalState.get(
			"thinkingBudgetTokens",
		);
		...
		const awsBedrockCustomSelected = await context.globalState.get(
			"awsBedrockCustomSelected",
		);
		const awsBedrockCustomModelBaseId = await context.globalState.get(
			"awsBedrockCustomModelBaseId",
		);
		const openRouterModelId =
			await context.globalState.get("openRouterModelId");
		const openRouterModelInfo = await context.globalState.get(
			"openRouterModelInfo",
		);
		...
		const requestyModelInfo =
			await context.globalState.get("requestyModelInfo");
		...
		const huggingFaceModelId =
			await context.globalState.get("huggingFaceModelId");
		const huggingFaceModelInfo = await context.globalState.get(
			"huggingFaceModelInfo",
		);

		// Read previous mode values
		const previousModeApiProvider = await context.globalState.get(
			"previousModeApiProvider",
		);
		const previousModeModelId = await context.globalState.get(
			"previousModeModelId",
		);
		const previousModeModelInfo = await context.globalState.get(
			"previousModeModelInfo",
		);
		const previousModeVsCodeLmModelSelector = await context.globalState.get(
			"previousModeVsCodeLmModelSelector",
		);
		const previousModeThinkingBudgetTokens = await context.globalState.get(
			"previousModeThinkingBudgetTokens",
		);
		const previousModeReasoningEffort = await context.globalState.get(
			"previousModeReasoningEffort",
		);
		const previousModeAwsBedrockCustomSelected = await context.globalState.get(
			"previousModeAwsBedrockCustomSelected",
		);
		const previousModeAwsBedrockCustomModelBaseId =
			await context.globalState.get("previousModeAwsBedrockCustomModelBaseId");
		const previousModeSapAiCoreModelId = await context.globalState.get(
			"previousModeSapAiCoreModelId",
		);
		...
		if (planActSeparateModelsSetting === false) {
			console.log(
				"Migrating with separate models DISABLED - using current values for both modes",
			);
			...
			console.log(
				"Migrating with separate models ENABLED - using current->plan, previous->act",
			);
		...
		console.log(
			"Successfully migrated legacy API configuration to mode-specific keys",
		);
	} catch (error) {
		console.error(
			"Failed to migrate legacy API configuration to mode-specific keys:",
			error,
		);
		// Continue execution - migration failure shouldn't break extension startup
	}
...
export async function migrateWelcomeViewCompleted(
	context: vscode.ExtensionContext,
) {
	try {
		// Check if welcomeViewCompleted is already set
		const welcomeViewCompleted = context.globalState.get(
		...
		if (welcomeViewCompleted === undefined) {
			...
			const openAiNativeApiKey =
				await context.secrets.get("openAiNativeApiKey");
			...
			const youApiKey = await context.secrets.get("youApiKey");
			...
			const planModeOllamaModelId = context.globalState.get(
				"planModeOllamaModelId",
			);
			const planModeLmStudioModelId = context.globalState.get(
				"planModeLmStudioModelId",
			);
			const actModeOllamaModelId = context.globalState.get(
				"actModeOllamaModelId",
			);
			const actModeLmStudioModelId = context.globalState.get(
				"actModeLmStudioModelId",
			);
			const planModeVsCodeLmModelSelector = context.globalState.get(
				"planModeVsCodeLmModelSelector",
			);
			const actModeVsCodeLmModelSelector = context.globalState.get(
				"actModeVsCodeLmModelSelector",
			);
			...
			console.log(
				`Migration: Set welcomeViewCompleted to ${hasKey} based on existing API keys`,
			);
		}
	} catch (error) {
...
export async function cleanupMcpMarketplaceCatalogFromGlobalState(
	context: vscode.ExtensionContext,
) {
	try {
		// Check if mcpMarketplaceCatalog exists in global state
		const mcpMarketplaceCatalog = await context.globalState.get(
		...
		if (mcpMarketplaceCatalog !== undefined) {
			...
			console.log(
				"Successfully removed mcpMarketplaceCatalog from global state",
			);
		}
	} catch (error) {
		console.error(
			"Failed to cleanup mcpMarketplaceCatalog from global state:",
			error,
		);
 ...
 */
export async function migrateHooksEnabledToBoolean(
	context: vscode.ExtensionContext,
) {
	try {
		...
		if (
			...
			typeof hooksEnabled === "object" &&
			hooksEnabled !== null
		) {
			console.log(
				"Migrating hooksEnabled from ClineFeatureSetting to boolean...",
			);
			...
			console.log(
				`Successfully migrated hooksEnabled to boolean: ${booleanValue}`,
			);
		}
	} catch (error) {
