...
export const config = createVariant(ModelFamily.NATIVE_GPT_5_1)
	...
	.overrideComponent(
		SystemPromptSection.AGENT_ROLE,
		gpt51ComponentOverrides[SystemPromptSection.AGENT_ROLE]!,
	)
	.overrideComponent(
		SystemPromptSection.RULES,
		gpt51ComponentOverrides[SystemPromptSection.RULES]!,
	)
	.overrideComponent(
		SystemPromptSection.TOOL_USE,
		gpt51ComponentOverrides[SystemPromptSection.TOOL_USE]!,
	)
	.overrideComponent(
		SystemPromptSection.ACT_VS_PLAN,
		gpt51ComponentOverrides[SystemPromptSection.ACT_VS_PLAN]!,
	)
	.overrideComponent(
		SystemPromptSection.OBJECTIVE,
		gpt51ComponentOverrides[SystemPromptSection.OBJECTIVE]!,
	)
	.overrideComponent(
		SystemPromptSection.FEEDBACK,
		gpt51ComponentOverrides[SystemPromptSection.FEEDBACK]!,
	)
	.build();

// Compile-time validation
const validationResult = validateVariant(
	{ ...config, id: ModelFamily.NATIVE_GPT_5_1 },
	{ strict: true },
);
if (!validationResult.isValid) {
	console.error(
		"GPT-5-1 variant configuration validation failed:",
		validationResult.errors,
	);
	throw new Error(
		`Invalid GPT-5-1 variant configuration: ${validationResult.errors.join(", ")}`,
	);
}

if (validationResult.warnings.length > 0) {
	console.warn(
		"GPT-5-1 variant configuration warnings:",
