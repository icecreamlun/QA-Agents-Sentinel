...
import { getAxolotlMdInstructions } from "@core/context/instructions/user-instructions/axolotl-md";
import {
	getGlobalClineRules,
	getLocalClineRules,
...
export class Task {
	...
	constructor(params: TaskParams) {
		...
		const openaiReasoningEffort = this.stateManager.getGlobalSettingsKey(
		...
		if (
			...
			if (mode === "plan") {
				effectiveApiConfiguration.planModeReasoningEffort =
					openaiReasoningEffort;
			} else {
				effectiveApiConfiguration.actModeReasoningEffort =
					openaiReasoningEffort;
			}
		}
		...
		const browserSettings =
			stateManager.getGlobalSettingsKey("browserSettings");
		const primaryModelSupportsImages =
			this.api.getModel().info.supportsImages ?? false;
		console.log(
			`[Axolotl Vision] Primary model supports images: ${primaryModelSupportsImages}`,
		);
		console.log(
			`[Axolotl Vision] Vision model provider configured: ${browserSettings.visionModelProvider}`,
		);
		if (!primaryModelSupportsImages && browserSettings.visionModelProvider) {
			...
			const visionApiKey = this.getVisionModelApiKey(
				browserSettings,
				apiConfiguration,
			);
			console.log(`[Axolotl Vision] Vision API key found: ${!!visionApiKey}`);
			if (visionApiKey) {
				this.visionApi = this.buildVisionApiHandler(
					browserSettings,
					visionApiKey,
				);
				console.log(
					`[Axolotl Vision] Vision API handler created: ${!!this.visionApi}`,
				);
			}
		}
		...
		const visionApiConfig: ApiConfiguration = {
			...
			planModeApiModelId:
				visionModelProvider === "anthropic"
					? visionModelId || defaultAnthropicModel
					: undefined,
			actModeApiModelId:
				visionModelProvider === "anthropic"
					? visionModelId || defaultAnthropicModel
			...
			planModeOpenAiModelId:
				visionModelProvider === "openai"
					? visionModelId || defaultOpenAiModel
					: undefined,
			...
			// OpenRouter
			openRouterApiKey:
				visionModelProvider === "openrouter" ? apiKey : undefined,
			planModeOpenRouterModelId:
				visionModelProvider === "openrouter" ? visionModelId : undefined,
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const providerInfo = this.getCurrentProviderInfo();
		const ide =
			(await HostProvider.env.getHostVersion({})).platform || "Unknown";
		const browserSettings =
		...
		console.log(
			`[Axolotl Vision] attemptApiRequest - modelSupportsBrowserUse: ${modelSupportsBrowserUse}, hasVisionModelFallback: ${hasVisionModelFallback}, disableBrowserTool: ${disableBrowserTool}`,
		);

		const supportsBrowserUse =
			(modelSupportsBrowserUse || hasVisionModelFallback) &&
			!disableBrowserTool; // only enable browser use if the model supports it (or we have a vision fallback) and the user hasn't disabled it
		...
		const localClineRulesFileInstructions = await getLocalClineRules(
			this.cwd,
			localToggles,
		);
		...
		const skillsEnabled =
			this.stateManager.getGlobalSettingsKey("skillsEnabled") ?? false;
		...
		const promptContext: SystemPromptContext = {
			...
			isCliSubagent,
			enableNativeToolCalls: this.stateManager.getGlobalStateKey(
		...
		const conversationHasImages =
			this.conversationContainsImages(truncatedHistory);
		const shouldUseVisionApi =
			!primaryModelSupportsImages && conversationHasImages && this.visionApi;
	...
	async recursivelyMakeClineRequests(
		...
		if (
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message:
						"Axolotl is having trouble. Would you like to continue the task?",
				});
			}
		...
		if (useAutoCondense && isNextGenModelFamily(this.api.getModel().id)) {
			...
			if (this.taskState.currentlySummarizing) {
				...
				const autoCondenseThreshold = this.stateManager.getGlobalSettingsKey(
		...
		try {
			...
			const assistantHasContent =
				assistantMessage.length > 0 || this.useNativeToolCalls;
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		if (isNextGenModel) {
			const autoCondenseThreshold =
				(this.stateManager.getGlobalSettingsKey("autoCondenseThreshold") as
					| number
					| undefined) ?? 0.75;
