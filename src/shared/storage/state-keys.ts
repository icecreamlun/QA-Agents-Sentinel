...
import {
	DEFAULT_FOCUS_CHAIN_SETTINGS,
	type FocusChainSettings,
} from "@shared/FocusChainSettings";
import type { HistoryItem } from "@shared/HistoryItem";
import {
	DEFAULT_MCP_DISPLAY_MODE,
	type McpDisplayMode,
...
const REMOTE_CONFIG_EXTRA_FIELDS = {
	...
	remoteMCPServers: {
		default: undefined as
			| Array<{ name: string; url: string; alwaysEnabled?: boolean }>
			| undefined,
	},
	...
	remoteGlobalRules: {
		default: undefined as GlobalInstructionsFile[] | undefined,
	},
...
const GLOBAL_STATE_FIELDS = {
	...
	dismissedBanners: {
		default: [] as Array<{ bannerId: string; dismissedAt: number }>,
	},
} satisfies FieldDefinitions;
...
const API_HANDLER_SETTINGS_FIELDS = {
	...
	planModeThinkingBudgetTokens: {
		default: ANTHROPIC_MIN_THINKING_BUDGET as number | undefined,
	},
...
const USER_SETTINGS_FIELDS = {
	...
	openTelemetryOtlpEndpoint: {
		default: "http://localhost:4318" as string | undefined,
	},
...
export type ApiHandlerOptionSettings = BuildInterface<
	typeof API_HANDLER_SETTINGS_FIELDS
>;
...
const GlobalStateAndSettingsKeySet = new Set(
	Object.keys(GLOBAL_STATE_AND_SETTINGS_FIELDS),
);
const ApiHandlerSettingsKeysSet = new Set(
	Object.keys(API_HANDLER_SETTINGS_FIELDS),
);
...
export const isAsyncProperty = (key: string): boolean =>
	ASYNC_PROPERTIES.has(key);
export const isComputedProperty = (key: string): boolean =>
	COMPUTED_PROPERTIES.has(key);

export const getDefaultValue = <K extends GlobalStateAndSettingsKey>(
	key: K,
): GlobalStateAndSettings[K] | undefined => {
	return ((GLOBAL_STATE_DEFAULTS as any)[key] ??
		(SETTINGS_DEFAULTS as any)[key]) as GlobalStateAndSettings[K] | undefined;
};

export const hasTransform = (key: string): boolean =>
	key in SETTINGS_TRANSFORMS;
export const applyTransform = <T>(key: string, value: T): T => {
	const transform = SETTINGS_TRANSFORMS[key];
	return transform ? transform(value) : value;
};

function extractDefaults<T extends Record<string, any>>(
	props: T,
): Partial<BuildInterface<T>> {
...
function extractTransforms<T extends Record<string, any>>(
	...
	return Object.fromEntries(
		Object.entries(props)
			.filter(
				([_, prop]) => "transform" in prop && prop.transform !== undefined,
			)
