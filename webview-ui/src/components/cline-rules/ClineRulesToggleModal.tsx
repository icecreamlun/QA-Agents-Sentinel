	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && (
						<TooltipContent>Manage Rules & Workflows</TooltipContent>
					)}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={
								isVisible ? "Hide Rules & Workflows" : "Show Rules & Workflows"
							}
				...
				>
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Axolotl with system-level guidance.
								...
								<p>
									Workflows allow you to define a series of steps to guide
									Axolotl through a repetitive set of tasks, such as deploying a
									service or submitting a PR. To invoke a workflow, type{" "}
								...
								<p>
									Skills are reusable instruction sets that Axolotl can activate
									on-demand. When a task matches a skill's description, Axolotl
									uses the <span className="font-bold">use_skill</span> tool to
									load the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points
									in Axolotl's execution lifecycle, enabling automation and
							...
							<>
								...
								{isWindows && (
									<div className="flex items-center gap-2 px-5 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
										...
										<span className="text-base">
											Hook toggling is not supported on Windows. Hooks can be
											created, edited, and deleted, but cannot be
											enabled/disabled and will not execute.
										</span>
									</div>
								)}
