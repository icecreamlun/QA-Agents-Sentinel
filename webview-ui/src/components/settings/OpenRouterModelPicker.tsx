...
const OpenRouterModelPicker: React.FC<OpenRouterModelPickerProps> = ({
	...
	const { handleModeFieldChange, handleModeFieldsChange, handleFieldChange } =
	...
	const [searchTerm, setSearchTerm] = useState(
		modeFields.openRouterModelId || openRouterDefaultModelId,
	);
	...
	const [activeTab, setActiveTab] = useState<"recommended" | "free">(() => {
		const currentModelId =
			modeFields.openRouterModelId || openRouterDefaultModelId;
		return freeModels.some((m) => m.id === currentModelId)
			? "free"
			: "recommended";
	});
	...
	const modelIds = useMemo(() => {
		...
		return filterOpenRouterModelIds(
			unfilteredModelIds,
			modeFields.apiProvider || "openrouter",
		);
	...
	return (
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				<>
					{showBudgetSlider && !showThinkingLevel && (
				...
				>
					Using OpenRouter preset: <strong>{searchTerm}</strong>. Preset models
					reference your configured model preferences on{" "}
					<VSCodeLink
				...
				>
					...
					If you're unsure which model to choose, Axolotl works best with{" "}
					<VSCodeLink
					...
					available. OpenRouter presets can be used by entering
