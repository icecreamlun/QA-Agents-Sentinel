import type { UpdateTerminalConnectionTimeoutResponse } from "@shared/proto/index.cline";
import {
	VSCodeCheckbox,
	VSCodeDropdown,
	VSCodeOption,
...
> = ({ renderSectionHeader }) => {
	...
	const [inputValue, setInputValue] = useState(
		(shellIntegrationTimeout / 1000).toString(),
	);
	...
	const handleExecutionModeChange = (event: Event) => {
		...
		const value =
			target.value === "backgroundExec" ? "backgroundExec" : "vscodeTerminal";
				...
				<div className="mb-5" id="terminal-settings-section">
					<div className="mb-4">
						...
						<p className="text-xs text-(--vscode-descriptionForeground) mt-1">
							Select the default terminal Axolotl will use. 'Default' uses your
							VSCode global setting.
						</p>
					</div>
					...
					<div className="mb-4">
						<div className="mb-2">
							...
							{inputError && (
								<div className="text-(--vscode-errorForeground) text-xs mt-1">
									{inputError}
								</div>
							)}
						</div>
						<p className="text-xs text-(--vscode-descriptionForeground)">
							Set how long Axolotl waits for shell integration to activate
							before executing commands. Increase this value if you experience
							terminal connection timeouts.
						</p>
					</div>
					...
					<div className="mb-4">
						...
						<p className="text-xs text-(--vscode-descriptionForeground)">
							When enabled, Axolotl will reuse existing terminal windows that
							aren't in the current working directory. Disable this if you
							experience issues with task lockout after a terminal command.
						</p>
					</div>
					{isVsCodePlatform && (
						<div className="mb-4">
							...
							<p className="text-xs text-[var(--vscode-descriptionForeground)] mt-1">
								Choose whether Axolotl runs commands in the VS Code terminal or
