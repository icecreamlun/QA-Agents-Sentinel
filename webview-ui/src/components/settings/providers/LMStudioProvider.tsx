...
export const LMStudioProvider = ({ currentMode }: LMStudioProviderProps) => {
	...
	const { lmStudioModelId } = getModeSpecificFields(
		apiConfiguration,
		currentMode,
	);
	...
	const requestLmStudioModels = useCallback(async () => {
		await ModelsServiceClient.getLmStudioModels({
			...
			.then((response) => {
				if (response?.values) {
					const models = response.values.map(
						(v) => JSON.parse(v) as LMStudioApiModel,
					);
	...
	const lmStudioMaxTokens =
		currentLMStudioModel?.max_context_length?.toString();
	const currentLoadedContext =
		currentLMStudioModel?.loaded_context_length?.toString();
	...
	return (
		<div className="flex flex-col gap-2">
			...
			<div className="text-xs text-description">
				...
				feature with <code>lms server start</code> to use it with this
				extension.{" "}
				<div className="text-error">
					<span className="font-semibold">Note:</span> Axolotl uses complex
